...
class Board
  ...
  def initialize(board = [[BLANK, BLANK, BLANK], [BLANK, BLANK, BLANK], [BLANK, BLANK, BLANK]])
    @board = board
  end
  ...
  def is_board_full?
    board.flatten.none? { |x| x == BLANK }
  end

  def free_space_count
    9 - board.flatten.filter { |x| x != BLANK }.length
  end
  ...
  private def find_winner_for_row(row)
    return row.first if row.uniq.length == 1 && row.none?(BLANK)
    return nil
  end
